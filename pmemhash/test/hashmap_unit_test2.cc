...
/*
 * Test hash_table with 'std::string' key type
 */
...
void print_ptr(int bucket)
{
    ...
    for (int i=0; i<4; i++) {
        ...
        if (e)
            printf("\tpos:%i, key:%s, value:%s\n", i, e->key.c_str(), e->value.c_str());
    }
}
...
int main(void)
{
    ...
    printf("put key:%s bucket:%i slot:%i value:%s\n", elem_1.key.c_str(), elem_ret.bucket_, elem_ret.slot_, elem_1.value.c_str());
    print_header(elem_ret.bucket_);
    elem_ret = my_hashtable.put(elem_1.key, &elem_2);
    printf("put key:%s bucket:%i slot:%i value:%s\n", elem_1.key.c_str(), elem_ret.bucket_, elem_ret.slot_, elem_2.value.c_str());
    print_header(elem_ret.bucket_);
    elem_ret = my_hashtable.put(elem_1.key, &elem_3);
    printf("put key:%s bucket:%i slot:%i value:%s\n", elem_1.key.c_str(), elem_ret.bucket_, elem_ret.slot_, elem_3.value.c_str());
    print_header(elem_ret.bucket_);
    
    elem_ret = my_hashtable.put(elem_4.key, &elem_4);
    printf("put key:%s bucket:%i slot:%i value:%s\n", elem_4.key.c_str(), elem_ret.bucket_, elem_ret.slot_, elem_4.value.c_str());
    print_header(elem_ret.bucket_);
