...
class TxLog {
    ...
    uint32_t getTxState(TxEntry *txEntry);
    ...
    bool getFirstPendingTx(uint64_t &id, boost::scoped_array<KVLayout> &readSet, boost::scoped_array<KVLayout> &writeSet);
    ...
    ///the class may allocate readSet and writeSet, caller responsible for destructing them
    bool getNextPendingTx(uint64_t &id, boost::scoped_array<KVLayout> &readSet, boost::scoped_array<KVLayout> &writeSet);
