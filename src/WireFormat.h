 ...
 */
#define QDB_MD_INITIAL  0
#define QDB_MD_INFINITY (uint64_t)-1
...
/*
 * SSN Information stored by the validator
 */
struct QDBXmitMeta {
    uint64_t pstamp;
    uint64_t sstamp;
...
/*
 * SSN Information generated by the client
 */
struct ClientQDBXmitMeta {
    uint64_t pstamp;
    uint64_t sstamp;
    __uint128_t cts;
} __attribute__((packed));
...
struct TxPrepare {
    ...
    enum OpType { READ, READONLY, REMOVE, WRITE, READ_MODIFY_WRITE, INVALID };
    ...
    struct Request {
        ...
        struct WriteOp {
            ...
            WriteOp(uint64_t tableId, uint64_t rpcId, uint32_t length,
		      RejectRules rejectRules, bool isRMW = false)
	        : type(isRMW ? OpType::READ_MODIFY_WRITE : OpType::WRITE)
                , tableId(tableId)
                , rpcId(rpcId)
                , length(length)
                , rejectRules(rejectRules)
